 ...
 * applications.services.smart-edge-open.docs/tree/main/components/networking/sriov-detection-app.md
*/
...
type CriteriaList struct {
	...
	HasIPV4Address       bool          `yaml:"hasIPV4Address"`
	HasNoIPV4Address     bool          `yaml:"hasNoIPV4Address"`
...
func CheckDevicesAgainstSpecificCriteriaList(criteriaListPtr *CriteriaList, devicesInfoSlice []SupDevice) int {
	...
	for deviceIndex, device := range devicesInfoSlice {
		...
		//DDP Support check
		if criteriaListPtr.DDPSupport == true && device.DDPSupport == false {
		...
		//Check if the device currently used - Up with IPV4 address
		if criteriaListPtr.HasIPV4Address == true && device.HasIPV4Address == false {
			PrintStdOut("The NIC with interface: %s does not have IPV4 Address! while the requirment does! So it is not fitting for the input EK to use for SR-IOV",
				device.InterfaceName)
			continue
		}
		//Check if the device currently NOT used - Up with IPV4 address
		if criteriaListPtr.HasNoIPV4Address == true && device.HasIPV4Address == true {
			PrintStdOut("The NIC with interface: %s has IPV4 Address! while the requirment does not! So it is not fitting for the input EK to use for SR-IOV",
				device.InterfaceName)
			continue
		}
...
******************************************************************************************************************/
func CheckVendorNameAndProductId(ptrPci *ghw.PCIInfo, busInfo string, deviceInterfaceName string) (bool, string) {
	deviceInfo := ptrPci.GetDevice(busInfo)
	if deviceInfo == nil {
		PrintStdOut("CheckVendorNameAndProductId: could not retrieve PCI device information for device: %s\n",
			deviceInterfaceName)
		return false, ""
	}
	...
	if len(product.ID) == 0 {
		PrintStdOut("CheckVendorNameAndProductId: Device %s has not Product Id! We will not use it for SR-IOV!\n",
			deviceInterfaceName)
		return false, ""
	}
	vendor := deviceInfo.Vendor
	if vendor.Name != INTEL_VENDOR_NAME {
		PrintStdOut("CheckVendorNameAndProductId: Device %s has vendor: %s which is not Intel! We will not use it for SR-IOV!\n",
			deviceInterfaceName, vendor.Name)
		return false, ""
	}
...
func FilteringDeviceForBasicCapabilities(ptrEthtoolHandle *ethtool.Ethtool, ptrPci *ghw.PCIInfo,
	...
	//Second check: Detect Vendor and Device Id
	result, deviceId = CheckVendorNameAndProductId(ptrPci, deviceBusInfo, networkDevice.Name)
	if result == false {
		return false, "", 0, "" //Returnning failure: result = false , empty deviceBusInfo, 0 maxVfs and empty deviceId
	}
...
func DetectDevices(ptrEthtoolHandle *ethtool.Ethtool, ptrPci *ghw.PCIInfo,
	...
	for _, networkDevice := range networkInterfaces {
		...
		hasIPV4Address := CheckIfDeviceCurrentlyUsed(networkDevice)
		...
		supDevice := SupDevice{networkDevice.Name, deviceBusInfo, deviceId, driver, linkSpeed, uint32(maxVfs),
			linkState, int8(numaNode), "", false, ptpSupport, rruConnection, specificPort, cardAffinity,
			hasIPV4Address, false, false}
