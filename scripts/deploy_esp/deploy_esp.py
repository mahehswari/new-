...
# Ensure that all the third-party dependencies are installed.
#  WARNING:
#   Following pylint checks have to be disabled because the preferred import order won't work with the dependencies
#   verification solution.
#   Take care for keeping the proper order when adding new imports
...
def parse_args(default_config_path, experience_kit_name):
    ...
    p.add_argument(
        "--run-esp-for-usb-boot", action="store_true", default=False,
        help="start ESP services for booting from USB image")
    p.add_argument(
        "--run-esp-for-pxe-boot", action="store_true", default=False,
        help="start ESP services for booting from PXE")
    p.add_argument(
        "--stop-esp", action="store_true", default=False,
    ...
    p.add_argument(
        "--log-dir", action="store", dest="log_dir_path", metavar="PATH",
        help="""
           PATH to the directory into which the command will put its logs; if the option is missing, the program won't
...
def get_version(command: str) -> tuple:
    ...
    try:
        ...
        raise seo.error.AppException(
            seo.error.Codes.MISSING_PREREQUISITE,
            f"Command '{command}' returned an error - {ex.returncode}:\n"
            f"    {ex}\n"
            f"    {seo.error.TS_REF}") from ex
...
def check_preconditions(args):
    ...
    for cmd in cmds:
        ...
        try:
            stream = subprocess.PIPE if args.debug else subprocess.DEVNULL
            subprocess.run(cmd, stdout=stream, stderr=stream, check=True) # nosec B602, B603
...
def configure_se_profile_customize_vars(profile, profile_path):
    ...
    for name, body, seo_path in items:
        ...
        if name == 'all':
            all_vars = seo.yaml.load( # nosec - B506 (project specific wrapper, safe_load used internally)
                seo_full_path)
...
def configure_se_profile_config_yaml(config, profile, profile_path):
    ...
    with open(profile_path / "conf/config.yml", "r+", encoding="utf-8") as f:
        ...
        dockerhub_user = config.get('docker', {}).get('dockerhub', {}).get('username')
        dockerhub_pass = config.get('docker', {}).get('dockerhub', {}).get('password')
        if dockerhub_user and dockerhub_pass:
            content = re.sub(
                r'(kernel_arguments:.*)$',
                f"\\1 docker_login_user={dockerhub_user} "
                f"docker_login_pass={dockerhub_pass}",
                content)
...
def configure_profile_settings(config, profile, profile_path):
    ...
    variables = {
        ...
        'git_password': config.get('git', {}).get('password', ''),
        'dhcp_client_address_freeze': config.get('dhcp_client_address_freeze', False)
    }
    ...
    if not is_profile_bare_os(profile):
        ek = profile['experience_kit']
        variables.update({
            'url':  ek['url'],
            'deployment': ek['deployment'],
            'branch': ek['branch'],
            'enable_lfs': ek.get('lfs', False)
        })
...
def configure_se_profile_sideload_files(profile, profile_path):
    ...
    def _filter(_, names):
        """Ignore filter for shutil.copytree to skip files starting with '.git'"""
        return [ name for name in names if name.startswith(".git")]
        ...
        for idx, item in enumerate(profile['sideload']):
            ...
            if file_path.is_file():
                ...
                for p in file_path.glob('*'):
                    ...
                    if p.is_dir():
                        shutil.copytree(p, sideload_target, ignore=_filter)
                    else:
                        if not p.name.startswith(".git"):
                            shutil.copyfile(p, sideload_target)
                        else:
                            logging.debug("Sideload: ignoring %s because it starts with .git", p.joinpath())
...
def clone_esp(config):
    ...
    seo.git.checkout(esp_config['branch'], dest)
...
def configure_esp(config):
    ...
    if 'admin_interface' in config:
        esp_cfg['interface'] = config['admin_interface']
...
def build_esp(config):
    ...
    for profile in config['profiles']:
        ...
        if 'url' in profile:
            cmd = ['./build.sh', '-S', '-l', str(name), '-k', 'clearlinux']
            run_esp_script(cmd, workdir)
        else:
    ...
    rebuild_boot_menu_cmd = ['./build.sh', '-S', '-P', '-k', 'clearlinux']
    ...
    if 'hosts' in config and config['hosts']:
        ...
        for host in config['hosts']:
            if 'mac' in host:   # no mac -> no special configuration
                macs = [host['mac']] if isinstance(host['mac'], str) else host['mac']
                host_variables = dict(variables)

                # variables that should be taken from hosts section only
                to_copy = ['name', 'machine_id', 'monitoring_enabled']
                for name in to_copy:
                    if name in host:
                ...
                for mac in macs:
                    ...
                    path = f"{profile_path}/files/seo/provision_settings_{mac.lower()}"
...
def main(args, root_path, default_config_path):
    ...
    seo.slo.logger.config(args.prog, args.debug, args.log_dir_path)
